...
class PapyBot:
    ...
    def check_response_validity_of_geoloc(
        ...
        formated_full_address = full_address_preformated.replace(
            "st ", "saint ").replace("ste ", "sainte ").replace("-", " ")
        location_title_preformated = remove_some_words_and_format_text(
            location_title, ["kjis552aCfd"])
        formated_location_title = location_title_preformated.replace(
            "st ", "saint ").replace("ste ", "sainte ").replace("-", " ")
        question = (question
            .replace("st ", "saint ")
            .replace("ste ", "sainte ")
    ...
    def get_geolocation(cls, question, city, message_from_papy):
        ...
        if response:
            ...
            if nbr_of_items_returned == 0:
                message_from_papy += (
                    "Je ne sais pas où se trouve ce lieu ")
            ...
            # If at least one item has been found
                ...
                for i, item in enumerate(items):
                    ...
                    if location_title:
                        print("location_title: ", location_title)
            ...
            if is_location_found:
                ...
                match = re.match(
                    reg_to_extract_city, city_extracted_from_full_address)
                ...
                name_out_of_street = (
                    extract_name_out_of_street(street, LOCATION_WORDS))
    ...
    def translate_points_categories(cls, interesting_points_list, language):
        for point in interesting_points_list:
            point["category"]["title"] = (
                cls.get_translation(point["category"]["title"],
                language.title())
                    .replace(
    ...
    def get_translation(cls, text_to_be_translated, language):
        ...
        return (
            GoogleTranslator(source='en', target=language)
            .translate(text_to_be_translated))

    @classmethod
    def turn_interesting_points_into_html_version(
            cls, interesting_points_list):
        interesting_points_list = (
            cls.html_version_of_one_interesting_point(interesting_points_list))
    ...
    def get_info_from_wikipedia(cls, question):
        ...
        if items_from_wikipedia:
            return (
                Wikipedia.get_item_info_from_wikipedia(
                    items_from_wikipedia[0])
                + (
    ...
    def start(cls, question: str) -> tuple:
        ...
        # 1 Get question =====================================================
        # ====================================================================
        print("question from front: ", question)
        message_from_papy = ""
        ...
        print(
            "formated_text = remove_some_words_and_format_text"
            "(text_to_format, WORDS_OF_COURTESY): ", formated_text)
        ...
        # print("le lieu à trouver est: ", question)
        # print("La ville probable est: ", city.capitalize())
        # print()
        ...
        if not location_title:
            ...
            message_from_papy += response_from_geoloc_function.get(
                "message_from_papy")
            message_from_papy += (
        ...
        interesting_points_list = GeolocApi.get_interesting_points_around(latitude, longitude)
        print()
        print("interesting_points_list = GeolocApi.get_interesting_points_around(latitude, longitude): ", interesting_points_list)
        interesting_points_str: str = None
        if interesting_points_list:
            interesting_points_list = cls.translate_points_categories(interesting_points_list, FRENCH)
            print()
            print("interesting_points_list = cls.translate_points_categories(interesting_points_list, FRENCH): ", interesting_points_list)
            interesting_points_str = cls.turn_interesting_points_into_html_version(interesting_points_list)
            print()
            print("interesting_points_str = cls.turn_interesting_points_into_html_version(interesting_points_list): ", interesting_points_str)
        ...
        if extracted_city:
            street_n_city = response_from_geoloc_function["street"] + " " + extracted_city
        else:
            street_n_city = response_from_geoloc_function["street"]

        print()
        print("question (arg of: get_info_from_wikipedia): ", question)
        wiki_of_initial_question = cls.get_info_from_wikipedia(question)
        print("wiki_of_initial_question = cls.get_info_from_wikipedia(question): ", wiki_of_initial_question)
        print()
        print("street_n_city (arg of: get_info_from_wikipedia): ", street_n_city)
        wiki_about_the_street_that_has_been_found = cls.get_info_from_wikipedia(street_n_city)
        print("wiki_about_the_street_that_has_been_found = cls.get_info_from_wikipedia(street_n_city): ", wiki_about_the_street_that_has_been_found)
        print()
        print("name_out_of_street (arg of: get_info_from_wikipedia): ", name_out_of_street)
        wiki_about_the_name_extracted_out_of_the_street = cls.get_info_from_wikipedia(name_out_of_street)
        print("wiki_about_the_name_extracted_out_of_the_street = cls.get_info_from_wikipedia(name_out_of_street): ", wiki_about_the_name_extracted_out_of_the_street)
        ...
        if wiki_about_the_street_that_has_been_found and wiki_about_the_street_that_has_been_found not in wiki_of_initial_question:
            message_from_papy += (
        ...
        message_from_papy += cls.display_map(api_key=api_key, latitude=latitude, longitude=longitude)
        message_from_papy = f"""{sarting_message}{message_from_papy}"""
        print()
        print()
        print("expected_value = message_from_papy, 200: ", "(", message_from_papy, ", ", 200, ")")
        return message_from_papy, 200
