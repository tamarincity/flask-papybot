...
class Wikipedia:
    ...
    def get_items_from_wikipedia(cls, question):
        items_from_wikipedia = requests.get(URL_SUGGESTED_ITEMS_FROM_WIKIPEDIA + question)
    ...
    def get_item_info_from_wikipedia(cls, item: dict):
        ...
        if title_of_wikipedia_page:
            item_info_from_wikipedia = requests.get(URL_ITEM_INFO_FROM_WIKIPEDIA + title_of_wikipedia_page).json()
